...
def get_possible_states(state: (int, int), max_rows: int, max_columns: int, goal_states: List[Tuple[int, int]],
                        is_utility: bool) -> (List[Tuple[int, int]], List[str]):
    possible_states = []
    named_direction = []

    if state in goal_states and is_utility:
        return possible_states, named_direction  # return possible state empty for goal states

    if state[0] != 0:
        # it can go up
        possible_states.append((state[0] - 1, state[1]))
        named_direction.append("up")

    if state[1] != 0:
        # it can go left
        possible_states.append((state[0], state[1] - 1))
        named_direction.append("left")

    if state[0] != max_rows - 1:
        # it can go down
        possible_states.append((state[0] + 1, state[1]))
        named_direction.append("down")

    if state[1] != max_columns - 1:
        # it can go right
        possible_states.append((state[0], state[1] + 1))
        named_direction.append("right")
    ...
    #     index += 1
    return possible_states, named_direction


def print_policy(policies, shape, utility_matrix=None, goal_states=None):
    policies_matrix = []
    i: int = 0
    while i < shape[0]:
        policies_matrix.append([])
        j: int = 0
        while j < shape[1]:
            if (i, j) not in goal_states:
                policies_matrix[i].append(policies[i, j])
            else:
                policies_matrix[i].append(utility_matrix[i, j])
            j += 1

        i += 1

    return policies_matrix
...
def update_calculations(policies, rw, value: np.array, goal_states: List[Tuple[int, int]]):
    ...
    while i < max_rows:
        j: int = 0
        while j < max_columns:
            possible_states, named_direction = get_possible_states((i, j), max_rows, max_columns, goal_states, True)
    ...
    while i < max_rows:
        j: int = 0
        while j < max_columns:
            possible_states, named_direction = get_possible_states((i, j), max_rows, max_columns, goal_states, False)
            state_scores = list(map(lambda state: new_value[state[0], state[1]], possible_states))
            policies[(i, j)] = named_direction[np.argmax(state_scores)]
            j += 1
        i += 1
...
def run_experiments(default_reward: float):
    goal_states: List[Tuple[int, int]] = [(0, 3), (1, 3), (2, 3), (1, 1)]
    ...
    print("policies: ")
    pprint(print_policy(policies, utility_matrix.shape, utility_matrix, goal_states))
